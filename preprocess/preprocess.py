...
def bracket_filter(sentence, mode="phonetic"):
    """Filter out phonetic or spelling brackets from the sentence.
    
    Example:
        (3학년)/(삼 학년)이 한다고 해서
        phonetic: 삼 학년이 한다고 해서
        spelling: 3학년이 한다고 해서
...
def special_filter(sentence, mode="phonetic", replace=None):
    """Filter out special characters from the sentence.
    
    Example:
        아/ 몬 소리야, 그건 또. b/ -> 아 몬 소리야 그건 또.
    """
    SENTENCE_MARK = ["?", "!", "."]
    NOISE = ["o", "n", "u", "b", "l"]
    EXCEPT = ["/", "+", "*", "-", "@", "$", "^", "&", "[", "]", "=", ":", ";", ","]
